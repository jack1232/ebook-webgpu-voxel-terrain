...
fn valueAt(index : vec3u) -> f32 {
    if (any(index >= vec3(ips.resolution-1, ips.resolution-1, ips.resolution-1))) { return 0.0; }
...
fn lerpTerrainColor(colors:array<vec3f,5>, ta:array<f32,6>, t:f32) -> vec3f {
    let len = 6u;
    var res = vec3(0.0);
...
fn addTerrainColors(tmin:f32, tmax:f32, t:f32) -> vec3f {
    ...
    return lerpTerrainColor(colors, ta, tt);
}

fn addVolcanoColors(tmin:f32, tmax:f32, t:f32) -> vec3f {
    let colors = array<vec3f, 11>(
        vec3(1.0, 0.992, 0.98),
